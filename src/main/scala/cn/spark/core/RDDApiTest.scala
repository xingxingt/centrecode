...
object RDDApiTest {
  ...
  def joinTransformation(sc: SparkContext) = {
    val classArray = Array(Tuple2("1", "spark"), Tuple2("2", "scala"), Tuple2("3", "hadoop"), Tuple2("4", "hive"), Tuple2("5", "java"))
    val scoreArray = Array(Tuple2("1", 99), Tuple2("2", 90), Tuple2("3", 89), Tuple2("4", 90))
    val classRdd = sc.parallelize(classArray)
    val scoreRdd = sc.parallelize(scoreArray)
    val joinRdd = classRdd.join(scoreRdd)
    joinRdd.collect().foreach(item => println(s"join:${item._1}----${item._2}"))
  }

  def leftOuterJoinTransformation(sc: SparkContext) = {
    val classArray = Array(Tuple2("1", "spark"), Tuple2("2", "scala"), Tuple2("3", "hadoop"))
    val scoreArray = Array(Tuple2("1", 99), Tuple2("2", 90), Tuple2("3", 89), Tuple2("4", 90))
    val classRdd = sc.parallelize(classArray)
    val scoreRdd = sc.parallelize(scoreArray)
    val joinRdd = classRdd.leftOuterJoin(scoreRdd)
    joinRdd.collect().foreach(item => println(s"leftOuterJoin:${item._1}----${item._2}"))
  }

  def rightOuterJoinTransformation(sc: SparkContext) = {
    val classArray = Array(Tuple2("1", "spark"), Tuple2("2", "scala"), Tuple2("3", "hadoop"), Tuple2("4", "hive"))
    val scoreArray = Array(Tuple2("1", 99), Tuple2("2", 90), Tuple2("3", 89), Tuple2("4", 90), Tuple2("5", 90))
    val classRdd = sc.parallelize(classArray)
    val scoreRdd = sc.parallelize(scoreArray)
    val joinRdd = classRdd.rightOuterJoin(scoreRdd)
  ...
  def cogroup1Transformation(sc: SparkContext) = {
    ...
    val rdd2 = sc.makeRDD(Array(("A", "a"), ("A", "5"), ("C", "c"), ("D", "d")), 2)
  ...
  def cogroup2Transformation(sc: SparkContext) = {
    ...
    val rdd3 = sc.makeRDD(Array(("A", "A"), ("E", "E"), ("F", "h")), 2)
    val rdd4 = rdd1.cogroup(rdd2, rdd3)
    rdd4.collect.foreach(item => println(s"cogroup2:${item._1}----${item._2}"))
  }

  /*rdd重分区*/
  def coalesceTransformation(sc: SparkContext): Unit = {
    val rdd1 = sc.makeRDD(Array(("A", "1"), ("B", "2"), ("C", "3")), 2)
    val rdd2 = rdd1.coalesce(1, false)
  ...
  def reduceByKeyDemo(sc: SparkContext): Unit = {
    ...
    //参数numPartitions用于指定分区数；
    //参数partitioner用于指定分区函数
    val rdd2 = rdd1.reduceByKey(new org.apache.spark.HashPartitioner(2), (x, y) => x + y)
    rdd2.collect().foreach(item => println(item))
